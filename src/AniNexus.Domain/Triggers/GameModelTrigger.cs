...
public class GameModelTrigger : BeforeSaveTriggerBase<GameModel>
{
    ...
    {
        DbContext.GameSystemRecommendations.RemoveRange(r => r.GameId == context.Entity.Id || r.GameRecommendationId == context.Entity.Id);
        DbContext.GameUserRecommendations.RemoveRange(r => r.GameId == context.Entity.Id || r.GameRecommendationId == context.Entity.Id);
    }

    private void DeleteRelated(ITriggerContext<GameModel> context)
    {
        DbContext.Set<GameRelatedMapModel>().RemoveRange(a => a.GameId == context.Entity.Id || a.RelatedGameId == context.Entity.Id);
    }
    ...
    private void CheckInsertPreconditions(ITriggerContext<GameModel> context)
    {
        ...
        if (!DbContext.ChangeTracker.Entries<MediaSeriesGameMapModel>().Any(e => e.Entity.GameId == context.Entity.Id && !IsEntityDeleted(e)))
        {
