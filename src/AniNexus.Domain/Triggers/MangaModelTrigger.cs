...
public class MangaModelTrigger : BeforeSaveTriggerBase<MangaModel>
{
    ...
    {
        DbContext.MangaSystemRecommendations.RemoveRange(r => r.MangaId == context.Entity.Id || r.MangaRecommendationId == context.Entity.Id);
        DbContext.MangaUserRecommendations.RemoveRange(r => r.MangaId == context.Entity.Id || r.MangaRecommendationId == context.Entity.Id);
    }

    private void DeleteRelated(ITriggerContext<MangaModel> context)
    {
        DbContext.Set<MangaRelatedMapModel>().RemoveRange(a => a.MangaId == context.Entity.Id || a.RelatedMangaId == context.Entity.Id);
    }

    private void CheckInsertPreconditions(ITriggerContext<MangaModel> context)
    {
        if (!DbContext.ChangeTracker.Entries<MangaReleaseModel>().Any(e => e.Entity.MangaId == context.Entity.Id && !IsEntityDeleted(e)))
        {
        ...
        if (!DbContext.ChangeTracker.Entries<MediaSeriesMangaMapModel>().Any(e => e.Entity.MangaId == context.Entity.Id && !IsEntityDeleted(e)))
        {
