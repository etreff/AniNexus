...
public class AnimeReleaseModelTrigger : BeforeSaveTriggerBase<AnimeReleaseModel>
{
    ...
    {
        var animeEntry = DbContext.ChangeTracker.Entries<AnimeModel>().FirstOrDefault(e => e.Entity.Id == context.Entity.AnimeId);
        var releasesForSameAnime = DbContext.ChangeTracker.Entries<AnimeReleaseModel>().Where(e => e.Entity.AnimeId == context.Entity.AnimeId).ToArray();
        ...
        if (context.ChangeType == ChangeType.Deleted)
        {
            ...
            DbContext.Set<MediaCompanyAnimeMapModel>().RemoveRange(m => m.ReleaseId == context.Entity.Id);
        }
        else
        {
        ...
        if (context.ChangeType == ChangeType.Modified && context.Entity.IsPrimary && context.UnmodifiedEntity is not null)
        {
            ...
            {
                foreach (var animeListEntry in DbContext.AnimeListEntries)
                {
