...
public class MangaReleaseModelTrigger : BeforeSaveTrigger<MangaReleaseModel>
{
    ...
    {
        var mangaEntry = DbContextFactory.ChangeTracker.Entries<MangaModel>().FirstOrDefault(e => e.Entity.Id == context.Entity.MangaId);
        var releasesForSameManga = DbContextFactory.ChangeTracker.Entries<MangaReleaseModel>().Where(e => e.Entity.MangaId == context.Entity.MangaId).ToArray();
        ...
        if (context.ChangeType == ChangeType.Deleted)
        {
            ...
            DbContextFactory.Set<MediaCompanyMangaMapModel>().RemoveRange(m => m.ReleaseId == context.Entity.Id);
        }
        else
        {
        ...
        if (context.ChangeType == ChangeType.Modified && context.Entity.IsPrimary && context.UnmodifiedEntity is not null)
        {
            ...
            {
                foreach (var mangaListEntry in DbContextFactory.MangaListEntries)
                {
