...
public readonly struct Array2D<T> : IEquatable<Array2D<T>>
{
    ...
    public Array2D(T[] array, int width, int height)
    {
        ...
        {
            ThrowHelper.ThrowInvalidOperationException($"The product of {nameof(width)} and {nameof(height)} must be less than or equal to the length of the memory region.");
        }
