...
namespace AniNexus.Data.GraphQL.Types.Primitives;
...
public class DateOnlyType : ScalarType<DateOnly, StringValueNode>
{
    ...
    public override bool TryDeserialize(object? resultValue, out object? runtimeValue)
    {
        ...
        if (resultValue is string text && TryDeserializeFromString(text, out var value))
        {
            runtimeValue = value;
            return true;
        }
    ...
    {
        if (DateOnly.TryParse(serialized, CultureInfo.InvariantCulture, DateTimeStyles.AssumeLocal, out var date))
        {
            result = date;
            return true;
        }
