...
namespace Microsoft.AspNetCore.Authentication.JwtBearer
{
    ...
    public class JwtBearerHandler : AuthenticationHandler<JwtBearerOptions>
    {
        ...
        /// </summary>
        /// <inheritdoc />
        public JwtBearerHandler(IOptionsMonitor<JwtBearerOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock, IRepositoryProvider repositoryProvider)
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            try
            {
                ...
                {
                    string[] issuers = new[] { _configuration.Issuer };
                ...
                foreach (var validator in Options.SecurityTokenValidators)
                {
                    if (validator.CanReadToken(token))
                    {
                        ...
                        {
                            Logger.TokenClaimMissing();

                            if (validationFailures == null)
                            {
                                validationFailures = new List<Exception>(1);
                            }
                            validationFailures.Add(new InvalidOperationException("Token is missing required claim."));
                            continue;
                        }

                        var repository = _repositoryProvider.GetUserRepository();
                        ...
                        if (user is null)
                        {
                            ...
                            validationFailures.Add(new InvalidOperationException("User not found."));
                        ...
                        if (user.IsBanned)
                        {
                            Logger.UserBanned(user.Username);

                            if (validationFailures == null)
                            {
                                validationFailures = new List<Exception>(1);
                            }
                            validationFailures.Add(new InvalidOperationException("User is banned."));
